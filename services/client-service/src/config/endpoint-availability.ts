...
export const SYSTEM_CAPABILITIES: Record<AyanamsaSystem, SystemCapabilities> = {
    lahiri: {
        ...
        specialCharts: ['moon', 'sun', 'sudarshan', 'arudha_lagna', 'bhava_lagna', 'hora_lagna', 'kp_bhava', 'equal_bhava', 'karkamsha'],
        hasDivisional: true,
...
export function isChartAvailable(system: AyanamsaSystem, chartType: string): boolean {
    ...
    return capabilities.charts.includes(chartType.toUpperCase());
}
...
export function getAvailableCharts(system: AyanamsaSystem): string[] {
    return SYSTEM_CAPABILITIES[system]?.charts || ['D1'];
}
