...
export class AdvancedCacheManager {
    ...
    private generateCacheKey(
        ...
        const normalized: Record<string, any> = {
            ...
            birthDate: typeof data['birthDate'] === 'string' ? data['birthDate'].split('T')[0] : data['birthDate'],
            // Normalize coordinates to 2 decimals
            latitude: data['latitude'] ? parseFloat((data['latitude'] as number).toFixed(2)) : undefined,
            longitude: data['longitude'] ? parseFloat((data['longitude'] as number).toFixed(2)) : undefined,
        };
    ...
    private recordHit(prefix: string, responseTime: number): void {
        ...
        const metrics = this.metrics.get(key) || { hits: 0, misses: 0, totalTime: 0, avgTime: 0 };
        metrics.hits++;
