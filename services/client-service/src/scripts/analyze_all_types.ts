...
async function analyze() {
  ...
  for (const type of YOGAS) {
    try {
      ...
      console.log(
        `| Yoga | ${type} | ${analysis.presenceKey} | ${analysis.value} | ${analysis.keys} |`,
      );
    } catch (e: any) {
  ...
  for (const type of DOSHAS) {
    try {
      ...
      const analysis = analyzeJson(data);
      console.log(
        `| Dosha | ${type} | ${analysis.presenceKey} | ${analysis.value} | ${analysis.keys} |`,
      );
    } catch (e: any) {
...
function analyzeJson(data: any): {
  ...
  if (!data || typeof data !== "object")
    return { presenceKey: "INVALID", value: "N/A", keys: "N/A" };

  const keys = Object.keys(data).join(", ");
  const presenceKey = "NOT_FOUND";
  const value: any = "N/A";
  ...
  for (const k of Object.keys(data)) {
    if (typeof data[k] === "object" && data[k] !== null) {
      ...
      if (result.presenceKey !== "NOT_FOUND") {
        return {
          presenceKey: `${k}.${result.presenceKey}`,
          value: result.value,
          keys,
        };
      }
