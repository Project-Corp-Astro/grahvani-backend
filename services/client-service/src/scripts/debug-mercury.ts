...
async function debugMercury() {
    ...
    try {
        ...
        console.log(`Mercury Maha: ${mercury.start_date} -> ${mercury.end_date} (Dur: ${mercury.duration_years})`);
        ...
        if (antars.length === 0) {
            ...
            if (mercuryAntar) {
                console.log(`  Mercury Antar: ${mercuryAntar.start_date} -> ${mercuryAntar.end_date} (Dur: ${mercuryAntar.duration_years})`);
                ...
                if (pratyantars.length > 0) {
                    ...
                    if (mercuryPrat) {
                        console.log(`    Mercury Pratyantar: ${mercuryPrat.start_date} -> ${mercuryPrat.end_date}`);
                    } else {
