...
async function debugDeepV2() {
    ...
    for (const v of variants) {
        console.log(`\nTesting level='${v}'...`);
        try {
            const res = await astroEngineClient.getVimshottariDasha(birthData, v, context);
            ...
            if (data.mahadashas) {
                ...
                if (maha && maha.antardashas) {
                    ...
                    console.log(`  Venus>Venus keys:`, antar ? Object.keys(antar) : 'Antar not found');
                    if (antar && (antar.pratyantardashas || antar.dasha_list || antar.sublevels)) {
                ...
                console.log('  Found dasha_list at root with length:', data.dasha_list.length);
                console.log('  Sample:', JSON.stringify(data.dasha_list[0]));
            } else if (data.pratyantardashas) {
                console.log('  Found pratyantardashas at root:', data.pratyantardashas.length);
            }
        } catch (e) {
