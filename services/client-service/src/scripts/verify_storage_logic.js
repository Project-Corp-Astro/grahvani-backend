...
async function storeYogaDoshaScanner(tenantId, clientId, category, type, system, analysisData) {
    ...
    // Clean data wrapper
    const storedData = analysisData?.data && typeof analysisData.data === 'object' && 'cached' in analysisData
        ? analysisData.data
        : analysisData;
    ...
    await prisma.clientYogaDosha.upsert({
        where: {
            tenantId_clientId_category_type_system: { tenantId, clientId, category, type, system }
        },
        update: { isPresent, analysisData: storedData, calculatedAt: new Date() },
        create: {
            tenantId, clientId, category, type, isPresent, system,
            analysisData: storedData, calculatedAt: new Date()
        }
    });
...
async function main() {
    ...
    try {
        ...
        for (const test of testCases) {
            ...
            await storeYogaDoshaScanner(TEST_TENANT, client.id, test.category, test.type, 'lahiri', test.data);
        ...
        try {
            ...
            await prisma.clientYogaDosha.deleteMany({ where: { clientId: client.id } });
