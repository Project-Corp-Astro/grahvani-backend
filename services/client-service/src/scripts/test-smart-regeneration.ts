...
async function testSmartRegeneration() {
    try {
        log('🚀 Starting Smart Regeneration Test...');
        ...
        log('✅ Dummy chart numerology_chaldean created (simulating old data)');

        // 3. Perform Non-Trigger Update (e.g., Phone number)
        log('🔄 Updating Phone (Should NOT trigger regeneration/deletion)...');
        await clientService.updateClient(TENANT_ID, client.id, {
            phonePrimary: `+91${Math.floor(1000000000 + Math.random() * 9000000000)}`
        }, metadata);

        // Verify chart still exists
        const chartsAfterPhoneUpdate = await chartRepository.findByClientId(TENANT_ID, client.id);
        const oldChartStillExists = chartsAfterPhoneUpdate.some(c => c.chartName === 'Old Numerology Chart');
        if (oldChartStillExists) {
            log('✅ Validated: Charts persist after non-critical update.');
        } else {
            log('❌ Failed: Charts were deleted after non-critical update!');
            log(`Found charts: ${chartsAfterPhoneUpdate.map(c => `${c.chartType}:${c.chartName}`).join(', ')}`);
            process.exit(1);
        }

        // 4. Perform Critical Update (e.g., Birth Time)
        log('🔄 Updating Birth Time (Should TRIGGER regeneration/deletion)...');
        ...
        const oldChartSurvives = chartsAfterCriticalUpdate.some(c => c.chartName === 'Old Numerology Chart');

        if (!oldChartSurvives) {
            log('✅ Validated: Old charts were deleted after critical update.');
        } else {
            log('❌ Failed: Old charts persist after critical update!');
            log(`Found charts: ${chartsAfterCriticalUpdate.map(c => `${c.chartType}:${c.chartName}`).join(', ')}`);
            process.exit(1);
        }
