...
async function debugCrashPath() {
    ...
    try {
        ...
        const contextLords = [context.mahaLord, context.antarLord, context.pratyantarLord, context.sookshmaLord];
        ...
        for (const lord of contextLords) {
            ...
            if (node) {
                ...
                const nextLevel = node.sublevels || node.antardashas || node.pratyantardashas || node.sookshmadashas || node.pranadashas;
        ...
        if (finalList.length === 0 && lastParent) {
            ...
            console.log(`Parent: ${lastParent.planet}, Start: ${lastParent.start_date}, Dur: ${lastParent.duration_years}`);
            if (!lastParent.start_date) console.error('CRITICAL: Parent has no start_date!');

            finalList = calculateSubPeriods(lastParent.planet, lastParent.start_date, lastParent.duration_years, lastParent.end_date);
