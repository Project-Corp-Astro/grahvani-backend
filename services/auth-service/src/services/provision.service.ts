...
export class ProvisioningService {
  ...
  async provision(data: ProvisionInput): Promise<ProvisionResult> {
    ...
    if (existingUser) {
      ...
      if (existingUser.status === "active") {
        logger.warn(
          { email: data.email },
          "Provision called for already active user",
        );
        return {
      ...
      if (existingUser.status === "pending_verification") {
        const newToken = await this.generateAndStoreInvitationToken(
          existingUser.id,
        );
        ...
        logger.info(
          { userId: existingUser.id },
          "Resent invitation for pending user",
        );
    ...
    logger.info(
      { userId: user.id, tenantId: data.tenantId },
  ...
  private async generateAndStoreInvitationToken(
    ...
    const expiresAt = new Date(
      Date.now() + this.INVITATION_TTL_DAYS * 24 * 60 * 60 * 1000,
    );
