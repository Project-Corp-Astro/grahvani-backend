  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      if (!token) {
        return res
          .status(400)
          .json({ active: false, error: "Token is required" });
      }
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const token = await tokenService.generateServiceToken(
        serviceName,
        permissions,
      );
...
router.post(
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      const { email, name, tenantId, role, features } =
        req.body as ProvisionInput;
      ...
      logger.info(
        { userId: result.userId, tenantId },
        "User provisioned via internal API",
      );
