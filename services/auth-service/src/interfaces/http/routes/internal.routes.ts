 ...
 */
router.post('/token/introspect', async (req: Request, res: Response, next: NextFunction) => {
    try {
 ...
 */
router.post('/token/validate', async (req: Request, res: Response, next: NextFunction) => {
    try {
    ...
    try {
        ...
        if (!serviceName) {
            return res.status(400).json({
                error: { code: 'INVALID_REQUEST', message: 'serviceName is required' }
            });
        }
