...
export class DivisionalController {
    ...
    async getDivisionalChart(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (cached) {
                res.json({ success: true, data: cached, cached: true, chartType: type, ayanamsa, calculatedAt: new Date().toISOString() });
                return;
            }
            ...
            res.json({ success: true, data: chartData, cached: false, chartType: type, ayanamsa, calculatedAt: new Date().toISOString() });
        } catch (error: any) {
