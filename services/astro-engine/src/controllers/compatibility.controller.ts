...
export class CompatibilityController {
    ...
    async getProgressed(req: Request, res: Response): Promise<void> {
        try {
            ...
            const data = await (await import('../services/astro-client')).astroEngineClient.getProgressedChart(birthData, progressedDate);
    ...
    async getWesternSynastry(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (cached) {
                res.json({ success: true, data: cached, cached: true, system: 'western', calculatedAt: new Date().toISOString() });
                return;
            }

            const data = await westernClient.getSynastry({ person1, person2 });
            await cacheService.set('western-synastry', cacheKey, data);
            res.json({ success: true, data, cached: false, system: 'western', calculatedAt: new Date().toISOString() });
        } catch (error: any) {
    ...
    async getWesternProgressed(req: Request, res: Response): Promise<void> {
        try {
            ...
            const cacheKey = { ...birthData, progressedDate, type: 'western-progressed' };
    ...
    private validateSynastryData(person1: BirthData, person2: BirthData, res: Response): boolean {
        ...
        if (!person2?.birthDate || !person2?.birthTime || !person2?.latitude || !person2?.longitude) {
            res.status(400).json({ success: false, error: 'Missing required birth data for person2' });
            return false;
        }
        return true;
    }
