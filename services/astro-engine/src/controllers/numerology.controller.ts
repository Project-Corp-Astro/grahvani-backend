...
export class NumerologyController {
    ...
    async getChaldeanNumerology(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!birthData.name) {
                res.status(400).json({ success: false, error: 'Name is required for Chaldean numerology' });
                return;
            }
    ...
    async getLoShuGrid(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!birthData.birthDate) {
                res.status(400).json({ success: false, error: 'Birth date is required for Lo Shu Grid' });
                return;
            }
