...
export const authMiddleware = async (
  ...
  try {
    ...
    if (tokenVersion < currentVersion) {
      console.warn(
        `[authMiddleware] Token version mismatch for user ${userId}. Token: ${tokenVersion}, Current: ${currentVersion}`,
      );
      return res.status(401).json({
        error: {
          code: "UNAUTHORIZED",
    ...
    if (error instanceof jwt.TokenExpiredError) {
      console.warn(
        `[authMiddleware] Token expired for user. ExpiredAt: ${error.expiredAt}`,
      );
