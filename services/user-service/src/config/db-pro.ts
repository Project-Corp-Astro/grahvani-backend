...
class CircuitBreaker {
    ...
    recordSuccess(): void {
        ...
        if (this.state === 'half-open') {
            ...
            if (this.successCount >= this.successThreshold) {
                this.state = 'closed';
                console.log({ breaker: this }, '✅ Circuit breaker CLOSED');
            }
        }
    }
    ...
    recordFailure(): void {
        ...
        if (this.failureCount >= this.failureThreshold) {
            this.state = 'open';
            console.warn({ breaker: this }, '🔴 Circuit breaker OPEN');
        }
    }
...
export class AdvancedDatabaseManager {
    ...
    private initializeClient(): void {
        try {
            console.log('🚀 Initializing Prisma client for Supabase PRO plan...');
            ...
            this.prismaClient.$use(async (params, next) => {
                ...
                try {
                    ...
                    if (cached) {
                        this.metrics.hitRate = (this.metrics.hitRate * 0.9) + (10 * 0.1);
                        return cached;
                    }
            ...
            console.error({ error }, '❌ Failed to initialize Prisma client');
            throw error;
        }
    }
    ...
    private getOptimalConnectionString(): string {
        ...
        if (process.env.DATABASE_URL) {
            ...
            if (url.includes(':6543/')) {
                console.log('✅ Using Transaction Mode Pooler (port 6543) - supports 200+ connections');
                return url;
            }
        }
        ...
        if (process.env.DATABASE_URL_POOLER) {
            console.log('✅ Using DATABASE_URL_POOLER (Transaction Mode) - supports 200+ connections');
        ...
        if (process.env.DIRECT_URL) {
            console.warn('⚠️ Using DIRECT_URL (Session Mode) - limited to ~15 connections! Consider using DATABASE_URL with port 6543');
        ...
        for (let attempt = 1; attempt <= this.poolConfig.maxRetries; attempt++) {
            try {
                ...
                if (attempt < this.poolConfig.maxRetries) {
                    ...
                    console.error({ attempts: attempt }, '❌ Database connection failed after retries');
    ...
    async healthCheck(): Promise<HealthCheck> {
        ...
        try {
            ...
            console.log({ healthCheck: this.lastHealthCheck }, '✅ Health check passed');
            ...
            console.error({ error, healthCheck: this.lastHealthCheck }, '❌ Health check failed');
            return this.lastHealthCheck;
        }
    }
    ...
    async disconnect(): Promise<void> {
        ...
        try {
            ...
            console.error({ error }, '❌ Error during database disconnect');
            throw error;
        }
    }
